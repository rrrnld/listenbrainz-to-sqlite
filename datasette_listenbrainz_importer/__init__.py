...
def upsert_artists_for_recording(db, artist_mbids, recording_mbid):
    for artist_mbid in artist_mbids:
        ...
        if not artist_on_recording.fetchone():
            db.execute('INSERT INTO artists_for_recordings VALUES ( ?, ? )', [ artist_mbid, recording_mbid ])
...
def upsert_listen(db, listen):
    imported_listen = db.execute('SELECT * FROM listens WHERE listened_at = ?', [ listen['listened_at'] ])
    if not imported_listen.fetchone():
        mbids = listen['track_metadata']['mbid_mapping']
        cur = db.execute('''
            INSERT INTO listens VALUES ( ?, ?, ?, ? )
        ''', [ listen['listened_at'], listen['user_name'], mbids['recording_mbid'], mbids['release_mbid'] ])

        db.executemany('''
            INSERT INTO listen_artists VALUES ( ?, ? )
        ''', [ (cur.lastrowid, artist_mbid) for artist_mbid in mbids['artist_mbids'] ])

@click.command(help='Imports listenbrainz history of a single user with associated artists, releases, and recordings.')
@click.option('--user', required=True, help='The Listenbrainz username form which to import listens', type=str)
@click.option('--max-results', help='Limit the maximum amount of fetched results.', type=int)
@click.option('--since', default=datetime.datetime(1970, 1, 1), help='Consider only listens more recent than this argument', type=click.DateTime())
@click.option('--until', default=datetime.datetime.now(), help='Consider only listens older than this argument', type=click.DateTime())
def import_listens(user, max_results, since, until):
    setup_database('listenbrainz.db')
    with sqlite3.connect('listenbrainz.db') as con, \
         tqdm(desc=f'Starting to fetch {max_results or "all"} listens from the listenbrainz API', unit=' listens') as pbar:
        ...
        while True:
            req = requests.get(f'https://api.listenbrainz.org/1/user/{user}/listens', { "max_ts": max_ts })
            if not req.ok:
                print(req)
                break
            ...
            for listen in body['listens']:
                if not listen['track_metadata'].get('mbid_mapping'):
                    print('Skipping listen without mbid info:', listen['track_metadata'].get('artist_name', ''), listen['track_metadata'].get('track_name', ''))
                    continue
